...
TEST(Lmdbxx, TestMappedSize) {
    ...
    cs::Connector::connect(&db->failed, [](const cs::LmdbException& e) {
        std::cout << "Error in database " << e.what();
    });

    std::cout << "Db map size " << db->mapSize();
    ...
    std::cout << "Key + value size " << key.size() + value.size();
...
TEST(Lmdbxx, TestAutoMapSizeIncrease) {
    ...
    std::cout << "Reallocates count " << reallocatesCount;
}
...
TEST(Lmdbxx, DISABLED_InsertInDifferentTables) {
    ...
    std::cout << "Size of " << db1 << " " << db->size(db1);
    std::cout << "Size of " << db2 << " " << db->size(db2);
